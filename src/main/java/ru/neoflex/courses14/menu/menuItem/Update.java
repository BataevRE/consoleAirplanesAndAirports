...
public class Update extends MenuItem {
    ...
    public void click(String command) {
        ...
        if (app.sessionGet("from").equals("airplanes")) {
            new AirplanesService().add(new MenuService().inputAirplaneFields((Long) app.sessionGet("airplane")));
        } else {
            try {
                new AirportsService().add(new MenuService().inputAirportFields((Long) app.sessionGet("airport")));
